...
if(isset($_POST["bayar"]) ) {
    ...
    $bayarDenda = $_POST["bayarDenda"];
    
    // Mengambil data denda dari database
    $queryDenda = queryReadData("SELECT denda FROM pengembalian WHERE id_pengembalian = $id_pengembalian");
    $dendaSebenarnya = $queryDenda[0]["denda"];
    
    // Membandingkan jumlah denda yang dibayarkan dengan jumlah denda yang sebenarnya
    if($bayarDenda == $dendaSebenarnya) {
        // Jika jumlah denda yang dibayarkan sesuai, lanjutkan dengan pembayaran
        ...
        // Jika jumlah denda yang dibayarkan tidak sesuai, tampilkan pesan kesalahan
        echo "<script>
        alert('Jumlah denda yang dibayarkan tidak sesuai. Silakan periksa kembali jumlah denda.');
        </script>";
    }
}
